...
import LinearAlgebra.SuperMatrix;
 ...
 * @author t.dang
 */
public class MSA_JamaMatrix {

    private final int q = 21;
    private final double pseudocount_weight = 0.5f;  // lamda
    private final double theta = 0.2f;
    ...
    public MSA_JamaMatrix(int m, int n, int[][] mx, String name) {
        M = m;
        N = n;
        AlgnMx = mx;
        Name = name;
    ...
    public MSA_JamaMatrix(int m, int n, String dir, String filename) throws FileNotFoundException, IOException {
        M = m;
        N = n;
        Dir = dir;
    ...
    public MSA_JamaMatrix(String dir, String filename) throws FileNotFoundException, IOException {
        AlgnMx = MyIO_DCA.ReturnAlignment(dir + filename);
        M = AlgnMx.length;
        N = AlgnMx[0].length;
        Dir = dir;
    ...
    public MSA_JamaMatrix(String dir, String filename, int[][] mx){
        AlgnMx = mx;
        M = AlgnMx.length;
        N = AlgnMx[0].length;
        Dir = dir;
    ...
    public MSA_JamaMatrix(MSA_JamaMatrix m){
...
//    }
    public ArrayList<MyOwnMatrix> Compute_mu(int i, int j, MyOwnMatrix W, MyOwnMatrix P1) {
        ...
        double diff = 1.0f;
        MyOwnMatrix mu1 = MyOwnMatrix.Ones(1, q).times(1f / q);
        MyOwnMatrix mu2 = MyOwnMatrix.Ones(1, q).times(1f / q);
...
//        System.exit(0);
        ...
        MyOwnMatrix invC = C.inverseMyOwnMatrix();
...
//                System.exit(1);
    ...
    public double[][] GetResult2SquareMatrix() throws IOException {
        int[] W_Km = this.Compute_W();
        // Print W_Km
        String str = Configuration.DirTest_DCA + this.Name.substring(0, 6);
//        MyIO.WriteToFile(str+".km", W_Km);
