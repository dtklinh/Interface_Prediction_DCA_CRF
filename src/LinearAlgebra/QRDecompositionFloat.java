...
public class QRDecompositionFloat implements java.io.Serializable {
 ...
 * ------------------------ */
   ...
   */
   ...
   public QRDecompositionFloat (FloatMatrix A) {
      // Initialize.
      QR = A.getArrayCopy();
      m = A.getRowDimension();
      ...
      for (int k = 0; k < n; k++) {
         ...
         float nrm = 0f;
         for (int i = k; i < m; i++) {
            nrm = (float) Maths.hypot(nrm,QR[i][k]);
         }
 ...
 * ------------------------ */
   ...
   */
   ...
   public FloatMatrix solve (FloatMatrix B) {
      ...
      return (new FloatMatrix(X,n,nx).getFloatMatrix(0,n-1,0,nx-1));
   }
